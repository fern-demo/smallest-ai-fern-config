...
| `format`              | string | `true`  | Enable punctuation and capitalization formatting in transcript responses. When `true`, transcripts include proper casing and punctuation (for example, `Hello, how are you?`). When `false`, returns raw lowercase text without punctuation (for example, `hello how are you`). Disable when feeding transcripts into downstream pipelines like LLMs that handle their own formatting. Options: `true`, `false` |
| `eou_timeout_ms`      | number | `800`   | End-of-utterance timeout in milliseconds. Controls how long the model waits after the user stops speaking before flushing the transcript as final. Lower values (for example, `300`) give faster responses for snappy voice agents. Higher values (for example, `2000`) accommodate users who pause mid-sentence. Range: `100` to `10000` |
...
url.searchParams.append("format", "true");
url.searchParams.append("eou_timeout_ms", "800");
...
async def stream_audio(audio_file, api_key, language="en", encoding="linear16", sample_rate=16000, word_timestamps="true", full_transcript="false", sentence_timestamps="false", redact_pii="false", redact_pci="false", numerals="auto", format="true", eou_timeout_ms=800, diarize="false"):
    params = {
        "language": language,
        "encoding": encoding,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eou-timeout-ms", type=int, default=800)
