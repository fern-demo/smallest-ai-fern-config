...
Cooldown period (in seconds) after the bot starts speaking during which user speech is ignored. Prevents the bot from being cut off prematurely — especially useful when the agent's first few words trigger the user's VAD.
